...
def find_optimal_differential_trail(cipher,  scenario = "single-key", technique="SMT", solver="Cadical"):
    ...
    # Fix the key difference to be zero, and the plaintext difference to be non-zero.
        fixed_values.append(set_fixed_variables('key', 'equal', list(range(cipher.KEY_BLOCK_SIZE)), integer_to_bit_list(0, cipher.KEY_BLOCK_SIZE, 'big')))
        fixed_values.append(set_fixed_variables('plaintext', 'not_equal', list(range(cipher.CIPHER_BLOCK_SIZE)), integer_to_bit_list(0, cipher.CIPHER_BLOCK_SIZE, 'big')))
    elif scenario == "related-key":
    # Fix the key difference to be non-zero
        fixed_values.append(set_fixed_variables('key', 'not_equal', list(range(cipher.KEY_BLOCK_SIZE)), integer_to_bit_list(0, cipher.KEY_BLOCK_SIZE, 'big')))
    if technique=="SMT":
        write(
            {
